...
export default function PhotoBox({name, title, description, avatar, big = false, className =''}: {
    ...
    return (
        <nav className={`flex flex-col gap-2 justify-center items-center ${big ? 'max-[25vw]:m-2 max-[25vw]:p-5 min-[25vw]:w-full min-[30vw]:h-[100vh]' : 'mb-6'} ${className}`}>
            ...
            <h2 className="max-[300px]:hidden font-bold font-sans text-center text-xl">{name}</h2>
            <div className={`${!big && 'hidden'} flex flex-col justify-center items-center`}>
                ...
                {big && <Button icon='' text="Know More" className="bg-blue-700 " link="portfolio"/>}
            </div>
        </nav>
    )
}
